...
E3 已完成真实三节点 Hadoop/YARN、MR 和 Spark checkpoint/失败恢复验证。CLI 端到端
...
- 当前 E3：同三节点部署 Hadoop 3.3.6 + Spark 3.4.4；HDFS 3 DataNode、YARN 3
  NodeManager，真实 MR 两种部署形态通过，YARN 日志聚合走 CephFS，Spark 三节点 checkpoint
  从 batch 0/40 行推进到失败后恢复的 batch 2/120 行。详见 [docs/E3-ENV.md](docs/E3-ENV.md)。
