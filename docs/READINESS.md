...
| H4 | 测试床供给 | 🟡 | E1 已恢复；E2 三节点已建、指纹已归档并有只读状态检查，仍需一键重建与 E3 | T08 | 可重复重建 |
...
1. **E3 与 E2 可重建性是第一阻塞**：E2 已在 `.44/.26/.28` 建成并通过 Release 全回归，
   第二网络、设备白名单和环境指纹已归档；但尚无一键重建/恢复脚本，也尚未部署 Hadoop/YARN
   发行版 E3。C/D/E/F 四个维度（共 55% 权重）的故障、性能、长稳和真实组件证据仍为空。
