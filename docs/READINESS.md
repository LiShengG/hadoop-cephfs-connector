...
> 一条不过就不能发 GA），可靠性提到 15%，因此加权结果下调到约 28%。以本表为准。
...
| ID | 子项 | 状态 | 要做的事（简述） | 阶段 | 达标判据 |
|---|---|:--:|---|---|---|
| B1 | Hadoop 契约 116 例 | ✅ | 已全绿（1 例书面 override） | — | — |
...
| ID | 子项 | 状态 | 要做的事（简述） | 阶段 | 达标判据 |
|---|---|:--:|---|---|---|
| F0 | **8 条 spike 证伪** | 🟡 | 探针已写好并自检通过，**就差在真集群上跑一次**（约 10 分钟） | SPIKE | 8 条均有结论 |
...
| H4 | 测试床供给 | ⛔ | E2/E3 一键起停脚本 + 环境指纹采集（无指纹的性能数据作废） | T08 | 可重复重建 |
...
1. **⛔ 物理资源是第一阻塞**：C/D/E/F 四个维度（共 55% 权重）全部依赖 E2（≥3 节点 Ceph）
   与 E3（Hadoop 发行版集群）。资源不到位，完成度停在 ~30% 不动，**与写多少代码无关**。
   这件事今天就能推进。
2. **最高性价比的下一步**：F0 的 8 条 spike——探针已就绪、自检通过，只需现有 vstart
   跑 10 分钟，就能把 20% 权重的生态维度从"全靠推断"变成"有结论"，且可能改变后续路线。
3. **可能的回退**：若 SP-01/SP-06 成立，需插入一轮产品增强（附录 A-9 owner/group 名字映射、
   A-10 安全口径），排期加 2–4 周。**这不是返工，是发现得早。**
