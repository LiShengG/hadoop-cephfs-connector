...
| **SP-04** | **Spark Structured Streaming checkpoint 原子创建**：`CheckpointFileManager` 依赖目标已存在时 rename 失败 | 普通文件 `Rename.NONE` 已用 MDS hard-link 原子抢占；并发 mkdirs EEXIST 复查为目录后幂等成功 | 多线程、多 JVM、崩溃恢复及 Spark 3.4.4 local checkpoint 已通过；目录 rename 与 E2/E3 仍是边界 | 在非 Debug E3 重跑双实例、重启及 Delta/Iceberg 提交 |
