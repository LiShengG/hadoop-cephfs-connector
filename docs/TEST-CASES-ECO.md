...
| **SP-04** | **Spark Structured Streaming checkpoint 语义不符**：`CheckpointFileManager` 依赖"目标已存在时 rename 失败"的原子创建；`CephFs` 全量委托，该路径走基类 `FileSystem.rename(src,dst,opts)` | `CephFileSystem.rename` 的目标检查与 `proto.rename` 分离，存在 TOCTOU | 默认端口缺陷已修复；8 线程连续 5 轮均有 2 个成功者，流作业 checkpoint 仍不可靠 | 增加原子 no-replace 能力后重跑并发与真实流作业 |
