...
> **本节为组件级冒烟；完整场景设计（88 条 + 8 条前置 spike）见
> [TEST-CASES-ECO.md](TEST-CASES-ECO.md)。** 下表 13 条是其中的 P0 骨架，
> 保留在此以便与其他类别一起做进度统计；执行时以 TEST-CASES-ECO.md 为准。
...
| SOAK | 5 | 100% 自动 + 监控采集 |
| ECO | 13（骨架）→ **实际 88 + 8 spike，见 TEST-CASES-ECO.md** | ≥ 80% 自动（Hive/Spark 用例可 SQL/脚本驱动） |
| COMPAT | 12 | 矩阵脚本驱动 |
| SEC | 10 | ≥ 70% 自动 |
| OPS | 10 | ≥ 50% 自动（发行版安装类可脚本化） |
| NEG | 8 | 100% 自动 |
| **合计** | **147 骨架 + 生态展开 83 = 230** | 整体自动化率目标 ≥ 80% |
